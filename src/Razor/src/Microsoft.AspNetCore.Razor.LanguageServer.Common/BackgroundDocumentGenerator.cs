...
namespace Microsoft.AspNetCore.Razor.LanguageServer.Common
{
    internal class BackgroundDocumentGenerator : ProjectSnapshotChangeTrigger
    {
        ...
        private readonly Dictionary<string, (ProjectSnapshot, DocumentSnapshot)> _work;
            ...
            IEnumerable<DocumentProcessedListener> documentProcessedListeners)
        {
            ...
            _work = new Dictionary<string, (ProjectSnapshot, DocumentSnapshot)>(StringComparer.Ordinal);
        }
        ...
        internal void Enqueue(ProjectSnapshot projectSnapshot, DocumentSnapshot document)
        {
            ...
            lock (_work)
            {
                ...
                _work[document.FilePath] = (projectSnapshot, document);
        ...
        private async void Timer_Tick(object state)
        {
            try
            {
                ...
                KeyValuePair<string, (ProjectSnapshot Project, DocumentSnapshot Document)>[] work;
                lock (_work)
                {
                    work = _work.ToArray();
                    _work.Clear();
                ...
                {
                    var document = work[i].Value.Document;
                    try
                    {
        ...
        private void NotifyDocumentsProcessed(KeyValuePair<string, (ProjectSnapshot Project, DocumentSnapshot Document)>[] work)
        {
            for (var i = 0; i < work.Length; i++)
            {
                foreach (var documentProcessedTrigger in _documentProcessedListeners)
                {
                    var item = work[i].Value;
                    documentProcessedTrigger.DocumentProcessed(item.Project, item.Document);
                }
            }
        }
                ...
                case ProjectChangeKind.ProjectAdded:
                    {
                        ...
                        foreach (var documentFilePath in projectSnapshot.DocumentFilePaths)
                        {
                            ...
                            Enqueue(projectSnapshot, document);
                        }
                ...
                case ProjectChangeKind.DocumentAdded:
                    {
                        ...
                        {
                            Enqueue(projectSnapshot, relatedDocument);
                        }
                ...
                case ProjectChangeKind.DocumentRemoved:
                    {
                        ...
                        foreach (var relatedDocument in olderProject.GetRelatedDocuments(document))
                        {
                            ...
                            Enqueue(args.Newer, newerRelatedDocument);
                        }
                        break;
                    }

...
namespace Microsoft.CodeAnalysis.Razor.Serialization
{
    internal class TagHelperDescriptorJsonConverter : JsonConverter
    {
        ...
        private TValue ReadValue<TValue>(JsonReader reader)
        {
            if (!MoveToContent())
            {
                throw new JsonSerializationException("Unexpected end when reading TagHelper value.");
            }
            ...
            ReadProperties(reader, propertyName =>
            {
                switch (propertyName)
                {
                    case nameof(TagHelperDescriptor.Documentation):
                        var documentation = reader.ReadAsString();
                        builder.Documentation = documentation;
                        break;
                    case nameof(TagHelperDescriptor.TagOutputHint):
                        var tagOutputHint = reader.ReadAsString();
                        builder.TagOutputHint = tagOutputHint;
                        break;
                    case nameof(TagHelperDescriptor.CaseSensitive):
                        var caseSensitive = reader.ReadAsBoolean();
                        builder.CaseSensitive = caseSensitive.Value;
                        break;
                    case nameof(TagHelperDescriptor.TagMatchingRules):
                        ReadTagMatchingRules(reader, builder, serializer);
                        break;
                    case nameof(TagHelperDescriptor.BoundAttributes):
                        ReadBoundAttributes(reader, builder, serializer);
                        break;
                    case nameof(TagHelperDescriptor.AllowedChildTags):
                        ReadAllowedChildTags(reader, builder, serializer);
                        break;
                    case nameof(TagHelperDescriptor.Diagnostics):
                        ReadDiagnostics(reader, builder.Diagnostics, serializer);
                        break;
        ...
        private static void ReadBoundAttributes(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
            ...
            do
            {
                ReadBoundAttribute(reader, builder, serializer);
            } while (reader.TokenType != JsonToken.EndArray);
        }

        private static void ReadBoundAttribute(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
                ...
                ReadProperties(reader, propertyName =>
                {
                    switch (propertyName)
                    {
                        ...
                        case nameof(BoundAttributeDescriptor.BoundAttributeParameters):
                            ReadBoundAttributeParameters(reader, attribute, serializer);
                            break;
                        case nameof(BoundAttributeDescriptor.Diagnostics):
                            ReadDiagnostics(reader, attribute.Diagnostics, serializer);
                            break;
        ...
        private static void ReadBoundAttributeParameters(JsonReader reader, BoundAttributeDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
            ...
            do
            {
                ReadBoundAttributeParameter(reader, builder, serializer);
            } while (reader.TokenType != JsonToken.EndArray);
        }

        private static void ReadBoundAttributeParameter(JsonReader reader, BoundAttributeDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
                ...
                ReadProperties(reader, propertyName =>
                {
                    switch (propertyName)
                    {
                        ...
                        case nameof(BoundAttributeParameterDescriptor.Diagnostics):
                            ReadDiagnostics(reader, parameter.Diagnostics, serializer);
                            break;
                    }
                });
            });
        ...
        private static void ReadTagMatchingRules(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
            ...
            do
            {
                ReadTagMatchingRule(reader, builder, serializer);
            } while (reader.TokenType != JsonToken.EndArray);
        }

        private static void ReadTagMatchingRule(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
                ...
                ReadProperties(reader, propertyName =>
                {
                    switch (propertyName)
                    {
                        ...
                        case nameof(TagMatchingRuleDescriptor.Attributes):
                            ReadRequiredAttributeValues(reader, rule, serializer);
                            break;
                        case nameof(TagMatchingRuleDescriptor.Diagnostics):
                            ReadDiagnostics(reader, rule.Diagnostics, serializer);
                            break;
                    }
                });
            });
        ...
        private static void ReadRequiredAttributeValues(JsonReader reader, TagMatchingRuleDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
            ...
            do
            {
                ReadRequiredAttribute(reader, builder, serializer);
            } while (reader.TokenType != JsonToken.EndArray);
        }

        private static void ReadRequiredAttribute(JsonReader reader, TagMatchingRuleDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
        ...
        private static void ReadAllowedChildTags(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
            ...
            do
            {
                ReadAllowedChildTag(reader, builder, serializer);
            } while (reader.TokenType != JsonToken.EndArray);
        }

        private static void ReadAllowedChildTag(JsonReader reader, TagHelperDescriptorBuilder builder, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;
                ...
                ReadProperties(reader, propertyName =>
                {
                    switch (propertyName)
                    {
                        ...
                        case nameof(AllowedChildTagDescriptor.Diagnostics):
                            ReadDiagnostics(reader, childTag.Diagnostics, serializer);
                            break;
                    }
                });
            });
        ...
        private static void ReadDiagnostics(JsonReader reader, RazorDiagnosticCollection diagnostics, JsonSerializer serializer)
        {
            if (!reader.Read())
            {
                return;

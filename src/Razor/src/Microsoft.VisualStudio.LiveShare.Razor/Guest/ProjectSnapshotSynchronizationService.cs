    ...
    internal class ProjectSnapshotSynchronizationService : ICollaborationService, IAsyncDisposable
    {
        ...
        internal void UpdateGuestProjectManager(ProjectChangeEventProxyArgs args)
        {
            if (args.Kind == ProjectProxyChangeKind.ProjectAdded)
            {
                ...
                var hostProject = new HostProject(guestPath, args.Newer.Configuration, args.Newer.RootNamespace, args.Newer.CSharpLanguageVersion);
            ...
            {
                var guestPath = ResolveGuestPath(args.ProjectFilePath);
                var hostProject = new HostProject(guestPath, args.Older.Configuration, args.Older.RootNamespace, args.Older.CSharpLanguageVersion);
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var projectHandle in projectHandles)
            {
                ...
                var hostProject = new HostProject(guestPath, projectHandle.Configuration, projectHandle.RootNamespace, projectHandle.CSharpLanguageVersion);
                _projectSnapshotManager.ProjectAdded(hostProject);

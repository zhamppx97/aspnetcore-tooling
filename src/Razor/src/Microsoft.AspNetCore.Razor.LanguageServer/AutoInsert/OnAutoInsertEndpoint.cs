...
namespace Microsoft.AspNetCore.Razor.LanguageServer.AutoInsert
{
    internal class OnAutoInsertEndpoint : IOnAutoInsertHandler
    {
        ...
        private readonly IReadOnlyList<RazorOnAutoInsertProvider> _onAutoInsertProvider;
            ...
            IEnumerable<RazorOnAutoInsertProvider> onAutoInsertProvider)
        {
            ...
            _onAutoInsertProvider = onAutoInsertProvider.ToList();
            _formatOnTypeTriggerCharacters = _onAutoInsertProvider.Select(provider => provider.TriggerCharacter).ToList();
        }
        ...
        public async Task<OnAutoInsertResponse> Handle(OnAutoInsertParams request, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < _onAutoInsertProvider.Count; i++)
            {
                var formatOnTypeProvider = _onAutoInsertProvider[i];
